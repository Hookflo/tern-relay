...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const isRelayRoot = host === 'relay.tern.dev' || host.startsWith('localhost')
    ...
    if (isWsUpgrade && isRelayRoot) {
      ...
      return stub.fetch(new Request(`https://session/connect?sessionId=${sessionId}`, request))
    }
    ...
    if (sessionId) {
      ...
      return stub.fetch(new Request(`https://session/forward${url.pathname}${url.search}`, request))
    }
...
export class SessionDurableObject extends DurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === '/connect') {
      ...
      const msg: RelayConnected = { type: 'connected', url: publicUrl, sessionId }
